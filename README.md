...
## Важно для проверки проекта

Другому разработчику не нужен мой API key.

Он должен создать **свой собственный Groq API key** и указать его в локальном `.env`.

Аналогично, PostgreSQL может быть локальным или предоставленным через любой совместимый PostgreSQL-хостинг. После указания `DATABASE_URL` достаточно применить Prisma migrations.
